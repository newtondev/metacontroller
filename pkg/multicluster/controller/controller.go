...
import (
	"context"
	"log"
	"metacontroller/pkg/multicluster/cluster"
	"metacontroller/pkg/multicluster/handler"
	"metacontroller/pkg/multicluster/manager"
	"metacontroller/pkg/multicluster/reconcile"
	"os"
	"time"
...
// Controller implementes the controller pattern.
...
// Options is ued as an argument of New.
type Options struct {
	// JitterPeriod is the time to wait after an error to start working again.
	JitterPeriod time.Duration
	// MaxConcurrentReconciles si the number of concurrent control loops.
...
func (c *Controller) WatchResourceReconcileOwner(ctx context.Context, cluster cluster.ClusterCache, groupVersionKind schema.GroupVersionKind, owner client.Object, ownerWatchOption WatchOptions) error {
	h := &handler.EnqueueRequestForOwner{
		...
		Filter:           ownerWatchOption.Filter,
		Predicates:       ownerWatchOption.Predicates,
	}
	return c.WatchResource(ctx, cluster, owner, h)
}

// WatchResourceReconcileObject configures the Controller to watch resources of the same Kind as objecType,
...
// in the specifed cluster, generating reconcile Requests an artitrary ResourceEventHandler.
