...
type Environment struct {
	...
	preinstallMetacontroller *bool
	loadImagesKindCluster    *bool
	kindClusterName          *string
	kindLogsLocation         *string

	selectedTestSelected *bool
...
func NewEnvironmentFromFlags() Environment {
	...
	c.createKindCluster = flag.Bool("create-kind-cluster", true, "create a kind cluster (and deploy Metacontroller) before running tests, if the cluster does not already exist with the same name")
	c.destroyKindCluster = flag.Bool("destroy-kind-cluster", true, "destroy the kind cluster when tests complete")
	c.preinstallMetacontroller = flag.Bool("preinstall-metacontroller", true, "install Metacontroller before running tests")
	c.loadImagesKindCluster = flag.Bool("load-images-kind-cluster", true, "load Metacontroller image into the kind cluster before running tests")
	...
	flag.Usage = func() {
		if f := flag.Lookup(testSuiteFlag); f != nil {
			f.Usage = fmt.Sprintf("%s. Available options: %+v", f.Usage, c.getAvailableSuiteOptions())
		}
...
func (e *Environment) getAvailableSuiteOptions() (opts []string) {
...
func (e *Environment) GetKindClusterName() string {
	...
	if *e.kindClusterName == "" {
		name := envconf.RandomName("metacontroller-e2e", 32)
...
// IsKindCluster returns true if the test is running agains a kind cluster.
func (e *Environment) IsKindCluster() bool {
	returns*e.createKindCluster || *e.kindClusterName != ""
}
...
// HelmUpgradeMetacontrollerToSuite returns a features.Func that upgrades metacontroller using
// the specified suite's helm install options.
func (e *Environment) HelmUpgradeMetacontrollerToSuite(suite string, extra ...helm.Option) env.Func {
	return funcs.HelmUpgrade(e.getSuiteInstallOpts(suite, extra...)...)
}

// HelmUpgradeMetacontrollerToBase returns a features.Func that upgrades metacontroller using
// the specified suite's helm install options.
func (e *Environment) HelmUpgradeMetacontrollerToBase() env.Func {
	return e.HelmUpgradeMetacontrollerToSuite(e.selectedTestSuite.String())
}

// HelmInstallBaseMetacontroller returns a features.Func that installs metacontroller using
// the default suite's helm install options.
func (e *Environment) HelmInstallBaseMetacontroller() env.Func {
...
// selected suite, appending additional specified ones.s
...
// HelmOptions valid for installing and upgrading the Metacontroller Helm chart.
// Used to install Metacontroller before any test starts, but some tests also use
// these options - for example to reinstall Metacontroller with a feature flag
// enabled.
...
// ShouldInstallMetacontroller returns true if the test should install Metacontroller
// before starting.
func (e *Environment) ShouldInstallMetacontroller() bool {
	return *e.preinstallMetacontroller
}
...
// for the selected suite, to be run before installing Metacontroller, if required.
